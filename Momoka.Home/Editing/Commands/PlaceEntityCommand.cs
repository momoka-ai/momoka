...
public sealed class PlaceEntityCommand : IEditorCommand
{
    ...
    public bool Execute(EditorSession session, out ChangeSet changes)
    {
        ...
        _entity = session.Residence.Entities.FirstOrDefault(e => e.Id == _entityId);
        if (_entity is null)
            return false;
