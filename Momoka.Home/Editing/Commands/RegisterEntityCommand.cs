...
/// <summary>
/// 登记命令（事务内部使用）：把新创建的实体注册进 <see cref="Residence.Entities"/>
...
public sealed class RegisterEntityCommand : IEditorCommand
{
    ...
    public bool Execute(EditorSession session, out ChangeSet changes)
    {
        ...
        if (session.Residence.Entities.Contains(_entity))
            return false;
        session.Residence.Entities.Add(_entity);
        return true;
    }
    ...
    {
        changes = new ChangeSet();
        return session.Residence.Entities.Remove(_entity);
    }
}
