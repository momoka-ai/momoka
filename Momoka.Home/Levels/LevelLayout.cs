...
/// space with root-absolute coordinates (no nested offset chains). LevelLayout
...
/// graphs).
/// </summary>
public sealed class LevelLayout : IEntitySource, IVoxelSource<Entity>, IEntityRelationSource
{
    ...
    public LevelLayout(VoxelLayout<Entity> voxelLayout, VoxelLayout<Region> regionLayout, List<Entity> entities)
    {
        Voxels = voxelLayout;
    ...
    public record class AtQuery(LevelLayout Source, Int3 Pos)
    {
    ...
    /// (dest voxels vs src voxels) — 实体对体积判定，委托 <see cref="Momoka.Home.Levels.Volumes.Volume.Intersects(Momoka.Home.Primitives.Int3, Momoka.Home.Levels.Volumes.Volume, Momoka.Home.Primitives.Int3)"/>。
    /// </summary>
    ...
    /// 待重写：低层写格应改为经过 LevelLayout 的受控通道（如事件 / 脏格跟踪），
    /// 而不是事后强制重建。重写后移除本方法。
    /// </remarks>
