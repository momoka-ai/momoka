...
/// <summary>
/// 客户端只读镜像（继承 <see cref="LevelData"/>：Type 为同步数据；基类 Entities /
/// Layout 客户端不维护——以自身 <see cref="Registry"/> 为实体真相）：由服务器快照 /
/// <c>layout_changed</c> 增量驱动，本地重建占用网格（实体列表是编辑器域唯一真相——
/// 从空图逐个放置）供射线 / 碰撞 / 框选预览。
/// **严格只读**：无任何写权威模型的入口——拖拽预览 = 只读查询，松手单次提交。
/// 实现 <see cref="IVoxelSource{T}"/> 以复用 Home 查询扩展（<c>FindItemsOnLine</c> 等）。
/// </summary>
public sealed class ClientLevelData : LevelData, IVoxelSource<Entity>
{
    /// <summary>全实体注册表（含未放置池与 Home 实体；客户端实体真相——基类 Entities 不维护）。</summary>
    ...
    {
        Type = Enum.TryParse<LevelType>(snapshot.Type, out var type) ? type : default;
        Registry.Clear();
        Placed.Clear();
