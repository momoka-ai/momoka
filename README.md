...
- **Momoka.Home — 空间数据模型**：
  ...
  - 实体系统：`Entity<T>`（`Int2` 瓦片 / `Int3` 体素内容 / `Float3` 连续活物）+ `Component` 行为载体（`DataSource` / `EventSource` / `CommandTarget` / `SurfaceSource`），已实现 `Wall`、`Door`、`Window`、`Appliance`、`Curtain` 等
