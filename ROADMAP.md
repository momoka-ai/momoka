...
- **互操作（2026-08-20 定案 + 2026-08-24 修订）**：SignalR 强类型——**不手写传输中间件**定案不变（中间件不放进插件；网关设施属 Core 宿主设施）；「单 Hub + 组合客户端契约」读法由 **Core 网关设施 · 三层通信模型（Events / Post / Packet）** 取代（2026-08-31 定案）；跨语言非目标（C# 主语言，Ui = Godot C# .NET）
...
- [ ] 与主机建立 SignalR 通信（Godot C# .NET 客户端，`Microsoft.AspNetCore.SignalR.Client`；经 Core 网关三层通信模型：Post 控制面查询 / Packet 数据面变更 / 事件由服务端广播）
...
- [x] **Commands Argument 极简化（2026-08-30）**：严格对齐 Minestom——Argument 仅 `Id` + `DefaultValue`（null = 必需，非 null = 可选），删 `IsOptional`/`HasDefaultValue`/`Optional()`/`PublishesValue`/`SyntaxString`/`ConsumesRemainingTokens`/`AllowSpace`/`UseRemaining`；值内空格由输入引号控制（`WordArgument` 并入 `StringArgument`）；可选性由 CommandSyntax 依缺省值控制（尾随省略 + 缺省注入）；**固定元数定案**——弃用 Minestom 的 useRemaining 式“消费剩余”（多余参数行为随命令漂移，破坏类型契约），每个槽位恰好一个 token、多余/缺失一律 InvalidSyntax，多值用类 JSON 数组字面量 `[a, b, c]` 表达；Commands 全域冗长调用 LINQ 紧凑化（热路径分词/匹配保留指令式）
